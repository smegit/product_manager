module DataImport
  class Import
    def initialize
      ...
      @products_path = "#{Rails.root}/files/products_test"

      # @product_images = "#{Rails.root}/public/pdf/images/products"

      @product_image_file_list = "#{Rails.root}/files/products_test/file_list"

      @product_images = "#{Rails.root}/files/products_test/file_list"

      @system_control_path = "#{Rails.root}/files/products_test/system_control"
    end
        ...
        @system_control_library.each do |row|
          if row[:control_type] == 'functions'
            ...
            prod_type_array.each do |t|
              system_control = SystemControl.create!(control_type: 'functions')

              system_control.update_attributes(value: { product_type: t, function_code: row[:code], description: row[:function_des], function_icon_url: row[:icon_url] })
            end
            ...
            system_control = SystemControl.create!(control_type: 'aesthetic')

            system_control.update_attributes(value: { aesthetic_code: row[:code], aesthetic_description: row[:des] })
    ...
    #   puts 'Starting Importing File List'

    #   counter = 0

    #   Dir.glob(@product_image_file_list + "/*.csv").each do |file|

    #     counter += 1

    #     puts "Importing Image List Files #{counter}/#{Dir.glob(@product_image_file_list + "/*.csv").count}"

    #     File.open(file, "r") do |file|

    #       @file_list = SmarterCSV.process(file, convert_values_to_numeric: false, remove_empty_values: false, remove_zero_values: false)

    #     end

    #     @file_list.each do |row|

    #       puts row

    #       if Product.exists?(model_number: row[:model_number])

    #         product = Product.find_by(model_number: row[:model_number])

    #         list = Attachment.create(attachment: row[:attachment])

    #         # list = Attachment.create({ :type => 'Jamie', :attachment => 'Attachment' }, :without_protection => true)
    ...
    #         puts "#{row[:model_number]} found with product_id = #{list.id}"

    #         product_attachment = ProductAttachment.create(product_id: product.id, attachment_id: list.id)

    #       else

    #         puts "#{row[:model_number]} not found"
    ...
    def import_images
      ...
      Dir.glob(@product_images + '/*.jpg').each do |image|
        ...
        puts File.open(image)

        product.image_attachments.create(attachment: File.open(image))

        next unless /(_+?HR)/.match(File.basename(image)).present?
    ...
    def data_hash(product)
      ...
      product.each do |k, v|
        if %i[function_list function_list_main_oven function_list_aux_oven].include?(k)
          ...
          when :function_list
            ...
            new_hash[k] = product[:function_list].split(',').map { |x| x.tr(' ', '_').underscore }

          when :function_list_main_oven

            new_hash[k] = product[:function_list_main_oven].split(',').map { |x| x.tr(' ', '_').underscore }

          when :function_list_aux_oven

            new_hash[k] = product[:function_list_aux_oven].split(',').map { |x| x.tr(' ', '_').underscore }
    ...
    def product_status(state)
      if %w[Active Current].include?(state)
